...
type Core<T extends {}> = Connect<T> & {getValue: () => T};

function createCore<T extends UnkownReactors>(reactors: T): Core<CoreResults<T>>
{
    type Map = [keyof T, Reactor<unknown, Action>];
    type State = CoreResults<T>;
    type SetState = (newState: CoreResults<T>) => void;

    const reactorArray = Object.keys(reactors).map((key): Map  => [key, reactors[key]]);
    const dispatch = (action: Action): void => reactorArray.forEach(([,rxr]) => rxr.dispatch(action));
    const connector = (connect as Core<CoreResults<T>>);
    connector.getValue = () => getValue();
    return connector;

    function connect<P>(component: RC<P>): RC<Without<P, T>>
    {
    ...
    {
        const i = {};
        return reactorArray.reduce((current, [key, rxr]) => ({...current, [key]: rxr.value}), i as CoreResults<T>);
    }

    function subscribe (state: State, setState: SetState): (_: Map) => Subscription
    {
        return ([key, rxr]) => rxr.subscribe(result => setState({...state, [key]: result}));
    }
}
